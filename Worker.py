...
HOST = '192.168.0.3'
PORT = 5000
BATCH_SIZE = 128   # imágenes por mini-batch — baja a 64 si sigues sin RAM
# ─────────────────────────────────────────────────────────────────────────────

_BUFFER_LIMIT = 100 * 1024 * 1024  # 100 MB
...
# ══════════════════════════════════════════════════════════════════════════════
# Gradientes MLP (numpy puro — no necesita batching, es ligero)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Gradientes CNN — con mini-batches para no agotar RAM
# ══════════════════════════════════════════════════════════════════════════════
...
def cnn_compute_gradients(X_np, y_np, weights_dict, model, device,
    ...
    """
    Calcula gradientes acumulados sobre todos los mini-batches.
    Nunca carga más de `batch_size` imágenes en memoria a la vez.
    """
    import torch
    import torch.nn.functional as F

    # Cargar pesos del servidor en el modelo local
    state_dict = {
    ...
    for start in range(0, n, batch_size):
        ...
        out  = model(X_batch)
        # Escalar la pérdida por el tamaño relativo del batch
        # para que el gradiente sea equivalente al de un forward completo
    ...
    avg_loss = total_loss   # ya está normalizado por n

    # Recolectar gradientes acumulados
    grads = {
    ...
    print(f"   Mini-batches procesados: {n_batches} "
          f"(batch_size={batch_size}, n={n})")

    return grads, float(avg_loss)
