...
# model.py - Agregar esta función al final
def compute_gradients(X: np.ndarray, y: np.ndarray, weights: Dict) -> Tuple[Dict, float]:
    """
    Calcula gradientes para un batch de datos.
    """
    m = X.shape[0]
    W1, b1 = weights["W1"], weights["b1"]
    W2, b2 = weights["W2"], weights["b2"]
    ...
    # Softmax estable
    expZ2 = np.exp(Z2 - np.max(Z2, axis=1, keepdims=True))
    A2 = expZ2 / np.sum(expZ2, axis=1, keepdims=True)
